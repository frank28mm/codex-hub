...
在 Codex Hub 中使用本 skill 时：

- 优先通过环境变量提供认证，而不是依赖 OpenClaw 配置文件：
  - `GETNOTE_API_KEY`
  - `GETNOTE_CLIENT_ID`
  - `GETNOTE_OWNER_ID`（可选）
- 如果当前环境变量缺失，不要假设存在 OpenClaw 的 `/note config` 命令；应明确提示用户补齐上述环境变量或本地配置后，再继续执行原请求。
- 其余 API 路径、路由规则和安全约束继续按下文原始 skill 约定执行。
